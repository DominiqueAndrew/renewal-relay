...
Evidence target: `074b275bc6d76993fb8e7d3b15c8699391e39b2d`
...
| Automated behavior | Local `npm test` | 10 passed, 0 failed |
| Policy conformance | Local `npm run eval` | 8/8 exact matches; status accuracy 1.0; review recall 1.0; ready precision 1.0 |
| Container build | `docker build --no-cache -t renewal-relay:lock-validation .` | Passed; lockfile-backed `npm ci` installed 128 production packages |
| Production dependency audit | `docker run --rm --entrypoint npm renewal-relay:lock-validation audit --omit=dev` | 0 vulnerabilities |
